...
#include "freertos/FreeRTOS.h"            // mutex types (camera frame-buffer lock)
...
#define SD_CS_PIN    10
// SPI clock for the card. The library default is 4 MHz, which is often too fast
// for a marginal slot or for an SD module on jumper wires -> "card not detected".
// 1 MHz is much more forgiving. If it STILL isn't detected, drop to 400000UL.
#define SD_SPI_HZ    1000000UL
SPIClass SD_SPI(HSPI);        // the card has its own dedicated SPI bus
bool     g_sd_ok = false;     // true once a card is mounted
uint32_t g_sd_count = 0;      // running counter -> keeps every saved filename unique
unsigned long g_lastSdTry = 0;// last remount attempt (loop retries until a card mounts)

// --- PIR motion sensor (HC-SR501). Connect the sensor's OUT (middle) pin here. ---
// ⚠️ PIN CHOICE MATTERS (confirmed vs. the HT-HC33 datasheet J2 header + pins_arduino.h):
//   - GPIO 45 is the CAMERA's data line (SIO_D / I2C SDA), driven the WHOLE time the
//     camera runs. Wiring the PIR there corrupts the camera — do NOT use it. (It's
//     also an ESP32-S3 strapping pin that sets the flash voltage at boot.)
//   - GPIO 2..9 are the onboard HaLow radio's internal SPI — off-limits.
//   - Everything else is the camera data bus or the SD card.
//   GPIO 1 (J2 pin 4) is free + broken out -> used here. It works on BOTH USB and
//   battery for reading the sensor. The only catch: GPIO 1 is also the board's
...
// and wedge the board. Take this before esp_camera_fb_get(), give it after
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    if (g_fb_mutex) xSemaphoreTake(g_fb_mutex, portMAX_DELAY);
    fb = esp_camera_fb_get();
    if (!fb) { if (g_fb_mutex) xSemaphoreGive(g_fb_mutex); res = ESP_FAIL; break; }
    ...
    if (g_fb_mutex) xSemaphoreGive(g_fb_mutex);
    if (res != ESP_OK) break;
  }
  return res;
}
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  if (!fb) { if (g_fb_mutex) xSemaphoreGive(g_fb_mutex); httpd_resp_send_500(req); return ESP_FAIL; }
...
//          4 = camera busy (couldn't get the frame-buffer lock in time).
// Every filename ends in a running counter so two photos in the same second
// (motion fires up to twice a second) never overwrite each other.
static int sd_capture_now(char *path_out, size_t path_len) {
  if (!g_sd_ok) return 1;

  bool locked = false;
  if (g_fb_mutex) {
    if (xSemaphoreTake(g_fb_mutex, pdMS_TO_TICKS(2000)) != pdTRUE) return 4;  // busy
    locked = true;
  }
  camera_fb_t *fb = esp_camera_fb_get();
  if (!fb) { if (locked) xSemaphoreGive(g_fb_mutex); return 2; }

  char path[64];
  uint32_t seq = ++g_sd_count;
  time_t now = time(NULL);
  if (now > 1700000000) {  // NTP set -> name by date/time + counter
    struct tm t; localtime_r(&now, &t);
    snprintf(path, sizeof(path), "/wildcam/wildcam_%04d%02d%02d_%02d%02d%02d_%lu.jpg",
             t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec,
             (unsigned long)seq);
  } else {                 // no time yet -> counter only
    snprintf(path, sizeof(path), "/wildcam/wildcam_%lu.jpg", (unsigned long)seq);
  }
  ...
  esp_camera_fb_return(fb);
  if (locked) xSemaphoreGive(g_fb_mutex);

  if (path_out && path_len) { strncpy(path_out, path, path_len); path_out[path_len - 1] = '\0'; }
  return ok ? 0 : 3;
}
...
static esp_err_t capture_sd_handler(httpd_req_t *req) {
  ...
  if (rc == 1) return httpd_resp_send(req, "NO_SD", 5);
  if (rc == 2) { httpd_resp_send_500(req); return ESP_FAIL; }
  if (rc == 4) return httpd_resp_send(req, "BUSY", 4);
  Serial.printf("SD save %s: %s\n", path, rc == 0 ? "ok" : "FAIL");
  return httpd_resp_send(req, rc == 0 ? "SAVED" : "WRITE_FAIL", rc == 0 ? 5 : 10);
}
...
void setup() {
  ...
  // Serialize access to the single camera frame buffer (stream vs. snapshot vs.
  // motion). Must exist before any web handler or the motion code can run.
  g_fb_mutex = xSemaphoreCreateMutex();

  // PIR motion sensor input. The SR501 actively drives OUT high/low, so the pin
  // needs no internal pull resistor.
  pinMode(PIR_PIN, INPUT);
  Serial.printf("[PIR] motion sensor on GPIO %d (warming up ~%lus before it arms)\n",
                PIR_PIN, PIR_WARMUP_MS / 1000UL);
  ...
  Serial.printf("[SD] mounting on HSPI  CLK=%d MISO=%d MOSI=%d CS=%d @ %lu Hz\n",
                SD_CLK_PIN, SD_MISO_PIN, SD_MOSI_PIN, SD_CS_PIN, SD_SPI_HZ);
  SD_SPI.begin(SD_CLK_PIN, SD_MISO_PIN, SD_MOSI_PIN, SD_CS_PIN);
  for (int attempt = 1; attempt <= 3 && !g_sd_ok; attempt++) {
    if (SD.begin(SD_CS_PIN, SD_SPI, SD_SPI_HZ)) {
      g_sd_ok = true;
  ...
  if (!g_sd_ok) Serial.println("[SD] not detected at boot — will keep retrying; camera/stream still work");
  g_lastSdTry = millis();
...
void loop() {
  ...
  // ---- Keep trying to mount the SD card until one shows up ----
  // Lets you insert/reseat a card after boot without re-flashing or rebooting.
  if (!g_sd_ok && (millis() - g_lastSdTry) > 3000) {
    g_lastSdTry = millis();
    SD.end();
    if (SD.begin(SD_CS_PIN, SD_SPI, SD_SPI_HZ)) {
      g_sd_ok = true;
      Serial.printf("[SD] card OK (%llu MB) — mounted on retry\n",
  ...
  delay(20);   // short, so motion is caught promptly and the 0.5 s cadence holds
}
